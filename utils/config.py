...
DEFAULTS = {
    ...
    "expression2": {
        "cc_number": 74, "zone_left": 0.10, "zone_right": 0.40,
    ...
    "chord_bank": {
        "enabled": False, "octave": 4, "active_preset": 0,
        "presets": [
            {"name": "Tu Me Dejaste", "chords": {1: "C", 2: "Em", 3: "D", 4: "B7", 5: "Am"}},
            {"name": "Pop I-V-vi-IV", "chords": {1: "C", 2: "G", 3: "Am", 4: "F", 5: "Em"}},
            {"name": "Jazz ii-V-I", "chords": {1: "Dm7", 2: "G7", 3: "Cmaj7", 4: "Am7", 5: "Fmaj7"}},
        ],
    },
}
...
@dataclass
class CameraConfig:
    index: int = 0; width: int = 640; height: int = 480

@dataclass
class DisplayConfig:
    scale: float = 1.5; window_name: str = "GestureChord v2"; start_in_debug: bool = False

@dataclass
class TrackingConfig:
    max_hands: int = 2; detection_confidence: float = 0.65; tracking_confidence: float = 0.55

@dataclass
class GestureConfig:
    hysteresis_high: float = 0.55; hysteresis_low: float = 0.35; rolling_window: int = 7

@dataclass
class StateMachineConfig:
    confirmation_frames: int = 5; change_frames: int = 4
    settle_frames: int = 3; release_grace_ms: int = 250
...
@dataclass
class MusicConfig:
    key: str = "C"; scale: str = "major"; octave: int = 4; velocity: int = 100

@dataclass
class ExpressionConfig:
    cc_number: int = 1; zone_top: float = 0.15; zone_bottom: float = 0.65
    smoothing: float = 0.25; dead_zone: float = 2.0; enabled: bool = True

@dataclass
class Expression2Config:
    cc_number: int = 74; zone_left: float = 0.10; zone_right: float = 0.40
    smoothing: float = 0.3; dead_zone: float = 2.0; enabled: bool = False

@dataclass
class MidiConfig:
    port_name: str = "GestureChord"; channel: int = 0

@dataclass
class ZoneConfig:
    threshold: float = 0.75; hand_lost_frames: int = 15

@dataclass
class VelocityConfig:
    enabled: bool = True; min_velocity: int = 50; max_velocity: int = 120
    speed_low: float = 0.003; speed_high: float = 0.04

@dataclass
class ArpeggiatorConfig:
    enabled: bool = False; bpm: float = 160.0; pattern: str = "up"; octave_range: int = 1

@dataclass
class RhythmConfig:
    enabled: bool = True; velocity_threshold: float = 0.010; cooldown_ms: float = 80.0
    smoothing: float = 0.5; min_velocity: int = 45; max_velocity: int = 120
    speed_for_max: float = 0.045

@dataclass
class GrooveConfig:
    enabled: bool = False; bpm: float = 120.0; pattern: str = "four_floor"
    gate_length: float = 0.85; humanize_ms: float = 10.0

@dataclass
class ChordBankConfig:
    enabled: bool = False
    octave: int = 4
    active_preset: int = 0
...
class Config:
    ...
    chord_bank: ChordBankConfig = field(default_factory=ChordBankConfig)
...
def _parse_presets(raw_presets) -> List[Dict]:
    """Parse preset list from YAML, handling string keys in chord maps."""
    if not isinstance(raw_presets, list):
        return DEFAULTS["chord_bank"]["presets"]
    result = []
    for p in raw_presets:
        ...
        for k, v in raw_chords.items():
            try:
                ...
                pass
        result.append({"name": name, "chords": chords})
    return result if result else DEFAULTS["chord_bank"]["presets"]


def _dict_to_config(data: dict) -> Config:
    cb_data = data.get("chord_bank", {})
    presets = _parse_presets(cb_data.get("presets", []))
    chord_bank = ChordBankConfig(
        enabled=cb_data.get("enabled", False),
        octave=cb_data.get("octave", 4),
        active_preset=cb_data.get("active_preset", 0),
        presets=presets,
    )
